<template>
  <div>
    <iframe v-if="fetchedBookID" frameborder="0" scrolling="no" class="zmax frame" :src="`https://books.google.co.kr/books?id=${fetchedBookID}&lpg=PP1&pg=PP1&output=embed&key='
    AIzaSyCrBZ5fHvUIZpsT8LzpSRhesRhE6pTeQk4'`" width="100%" height="100%" />
    <v-btn v-if="fetchedBookID" bottom right fixed color="primary" class="zmax" @click="fetchedBookID = ''">
      닫기
    </v-btn>
    ...
    <div class="text-center my-10">
      <div v-if="post.uid === userInfo.uid" class="mb-10">
        <LazyDialogComponent :cb="del" btn-title="삭제" title="진짜로 삭제하겠습니까?" text="삭제하면 복구할 수 없습니다" icon="trash-can" />
      ...
      <v-dialog width="700" v-if="post.isbn">
        ...
        <v-card>
          <v-card-text>
            ...
            <v-card-subtitle v-text="otherInfo.volumeInfo?.authors.join(', ')" />
    ...
    <LazyCommentComponent :link="`/book/content/${time}`" :dbr="`contents/${time}/comments`" :uid="this.post.uid" />
